...
# 2. chunking
def chunked_weather_data(embedding_executor):
    results = fetch_weather_data()
    return [embedding_executor.create_chunked_weather(results)]

# 3. Embedding
def embedding_weather_data():
    ...
    chunked_text_list = chunked_weather_data(embedding_executor)
    chunked_html = []
...
# 4. indexing
def indexing_weather_data():
    connect_to_milvus()
    collection_name = "weather_hereforus"
    ...
    # 데이터 준비
    chunked_html = embedding_weather_data()
    source_list = []
    text_list = []
    embedding_list = []
    ...
    utility.index_building_progress("weather_hereforus")
